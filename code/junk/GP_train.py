...
def rbf_kernel_D_vectorized(mat1, mat2, sigma, v0):
...
K_ss = rbf_kernel_D_vectorized(np.matrix(Xtest), np.matrix(Xtest), sigma, v0)
...
K = rbf_kernel_D_vectorized(np.matrix(Xtrain), np.matrix(Xtrain), sigma, v0)
L = np.linalg.cholesky(K + 0.00005*np.eye(len(Xtrain)))

 
# Compute the mean at our test points.
K_s = rbf_kernel_D_vectorized(np.matrix(Xtrain), np.matrix(Xtest), sigma, v0)
